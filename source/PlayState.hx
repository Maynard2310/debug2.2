...
class PlayState extends MusicBeatState
{
	...
	// Seaside Serenade Variables
	var creatures:BGSprite;
	var firstBeachLeaves:BGSprite;
	var firstBeachTrees:BGSprite;
	var frontCoral:BGSprite;
	var backCoral:BGSprite;
	var rockFlowers:BGSprite;
	var bubble:BGSprite;
	var fishies:BGSprite;
	var jellies:BGSprite;
	var fishiesTween:FlxTween;
	var jelliesTween:FlxTween;
	var firstSeaOverlay:BGSprite;
	var firstSeaBlend:BlendModeEffect;
	var firstBeachBirds:BGSprite;
	var birdsTween:FlxTween;
	var dolphins:BGSprite;
	var seals:BGSprite;
	var dolphinsTween:FlxTween;
	var sealsTween:FlxTween;
	var dolphinsAngle:FlxTween;
	var sealsAngle:FlxTween;
	var cutTime = 0;
	public var sEnding = 'none';
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'palmy-breeze' | 'white-sands':
					curStage = 'beachOne';
				case 'ocean-blue':
					curStage = 'beachOneOcean';
		...
		switch (curStage)
		{
			case 'beachOne': // Seaside Serenade - Palmy Breeze, White Sands

				// GameOverSubstate.deathSoundName = 'fnf_loss_sfx-pixel';
				// GameOverSubstate.loopSoundName = 'gameOver-pixel';
				// GameOverSubstate.endSoundName = 'gameOverEnd-pixel';
				GameOverSubstate.characterName = 'bf-sss-dead';

				var bg:BGSprite = new BGSprite('beachnumberone/sun', -400, -150, 0.1, 0.1);
				bg.setGraphicSize(Std.int(bg.width * 1));
				add(bg);

				firstBeachBirds = new BGSprite('beachnumberone/birds', -150, 0, 0.3, 0.3, ['birds'], true);
				firstBeachBirds.setGraphicSize(Std.int(firstBeachBirds.width * 0.5));
				firstBeachBirds.angle = 5.0;
				add(firstBeachBirds);

				birdsTween = FlxTween.linearPath(firstBeachBirds, [FlxPoint.get(-350, 50), FlxPoint.get(2000, -55)], FlxG.random.float(20, 25), true,
					{type: FlxTweenType.LOOPING, startDelay: FlxG.random.float(15, 25), loopDelay: FlxG.random.float(15, 25)});
				birdsTween.active = false;

				var firstBeachIsland:BGSprite = new BGSprite('beachnumberone/island', -550, 35, 0.3, 0.3);
				firstBeachIsland.setGraphicSize(Std.int(firstBeachIsland.width * 0.8));
				add(firstBeachIsland);

				var firstBeachOcean:BGSprite = new BGSprite('beachnumberone/ocean', -400, 200, 0.5, 0.3);
				add(firstBeachOcean);

				var firstBeachBG:BGSprite = new BGSprite('beachnumberone/Beach', -400, 300, ['beach'], true);
				add(firstBeachBG);

				creatures = new BGSprite('beachnumberone/Creature_Bop', -17, 500, 0.9, 0.9, ['Creature_Bop']);
				creatures.setGraphicSize(Std.int(creatures.width * 1));
				creatures.updateHitbox();
				add(creatures);

				firstBeachLeaves = new BGSprite('beachnumberone/leaves', -550, 700, 0.95, 0.95);
				firstBeachLeaves.setGraphicSize(Std.int(firstBeachLeaves.width * 1));
				// add(firstBeachLeaves);

				firstBeachTrees = new BGSprite('beachnumberone/trees', -455, -110, 0.92, 0.92);
				firstBeachTrees.setGraphicSize(Std.int(firstBeachTrees.width * 1.0));

			case 'beachOneOcean': // Seaside Serenade - Ocean Blue

				GameOverSubstate.characterName = 'bf-sss-dead';

				var bg:BGSprite = new BGSprite('beachnumberone/sea', -400, -150, 0.1, 0.1);
				bg.setGraphicSize(Std.int(bg.width * 1));
				add(bg);

				dolphins = new BGSprite('beachnumberone/dolphins', -400, 110, 0.2, 0.2);
				dolphins.setGraphicSize(Std.int(dolphins.width * 0.5));
				add(dolphins);

				dolphinsTween = FlxTween.quadPath(dolphins, [FlxPoint.get(-400, 110), FlxPoint.get(350, -400), FlxPoint.get(1100, 110),], 3, true, {
					type: FlxTweenType.LOOPING,
					startDelay: 23,
					loopDelay: FlxG.random.float(1, 4),
					onStart: function(twn:FlxTween)
					{
						FlxTween.angle(dolphins, -30, 90, 3);
						// {type: FlxTweenType.LOOPING});
					}
				});
				dolphinsTween.active = false;

				//	dolphinsAngle = FlxTween.angle(dolphins, -50, 80, 7,
				//	{type: FlxTweenType.LOOPING, startDelay: 22, loopDelay: FlxTween.dolphinsTween});

				var firstBeachSeaRocks:BGSprite = new BGSprite('beachnumberone/seaRocks', -550, 35, 0.3, 0.3);
				firstBeachSeaRocks.setGraphicSize(Std.int(firstBeachSeaRocks.width * 0.8));
				add(firstBeachSeaRocks);

				seals = new BGSprite('beachnumberone/seals', 1200, -300, 0.2, 0.2);
				seals.setGraphicSize(Std.int(seals.width * 0.5));
				add(seals);

				sealsTween = FlxTween.quadPath(seals, [FlxPoint.get(1200, -300), FlxPoint.get(500, 50), FlxPoint.get(-500, -200),], 3, true, {
					type: FlxTweenType.LOOPING,
					startDelay: 21,
					loopDelay: FlxG.random.float(1, 4),
					onStart: function(twn:FlxTween)
					{
						FlxTween.angle(seals, -30, 40, 3);
						// {type: FlxTweenType.LOOPING});
					}
				});
				sealsTween.active = false;

				var firstSeaGrass = new BGSprite('beachnumberone/seaGrass', -550, 220, 0.4, 0.9, ['sea grass'], true);
				// firstSeaGrass.setGraphicSize(Std.int(rockFlowers.width * 0.85));
				add(firstSeaGrass);

				var firstSeaFloor:BGSprite = new BGSprite('beachnumberone/seaFloor', -400, 300);
				add(firstSeaFloor);

				var firstSandRocks:BGSprite = new BGSprite('beachnumberone/sandRocks', -500, 320, 0.5, 0.9);
				add(firstSandRocks);

				fishies = new BGSprite('beachnumberone/fishies', -500, 130, 0.9, 0.9, ['feesh'], true);
				fishies.active = true;
				add(fishies);

				// fishiesTween = FlxTween.tween(fishies, {x: -500 - -2000 }, 7, {type: FlxTweenType.LOOPING, startDelay: 17, loopDelay: FlxG.random.float(4, 8 )});
				fishiesTween = FlxTween.linearPath(fishies, [FlxPoint.get(-300, 75), FlxPoint.get(1500, 75)], 7, true,
					{type: FlxTweenType.LOOPING, startDelay: 16, loopDelay: FlxG.random.float(2, 7)});
				fishiesTween.active = false;

				jellies = new BGSprite('beachnumberone/jellies', 800, 250, 0.9, 0.9, ['jellyfish_go_brrrrr'], true);
				jellies.setGraphicSize(Std.int(jellies.width * 0.8));
				jellies.active = true;
				add(jellies);

				// jelliesTween = FlxTween.tween(jellies, {x: 1 - 1.5 , y: 150 - 400,} , 4, {type: FlxTweenType.LOOPING});

				jelliesTween = FlxTween.linearPath(jellies, [FlxPoint.get(800, 250), FlxPoint.get(700, -350)], 5, true,
					{type: FlxTweenType.LOOPING, startDelay: 20, loopDelay: FlxG.random.float(7, 15)});
				jelliesTween.active = false;

				backCoral = new BGSprite('beachnumberone/backCoral', -300, 200, 0.9, 0.9);
				// bottomBoppers.animation.addByPrefix('hey', 'Bottom Level Boppers HEY', 24, false);
				backCoral.setGraphicSize(Std.int(backCoral.width * 1));
				backCoral.updateHitbox();
				add(backCoral);

				rockFlowers = new BGSprite('beachnumberone/rockFlowers', -530, -130, ['Front Foliage'], true);
				rockFlowers.setGraphicSize(Std.int(rockFlowers.width * 0.90));
				add(rockFlowers);

				frontCoral = new BGSprite('beachnumberone/frontCoral', -500, 700);
				frontCoral.setGraphicSize(Std.int(frontCoral.width * 1));
				// add(firstBeachLeaves);

				bubble = new BGSprite('beachnumberone/bubble', 700, 250);

				firstSeaOverlay = new BGSprite('beachnumberone/sea', -400, -150, 0.1, 0.1);
				firstSeaOverlay.setGraphicSize(Std.int(bg.width * 1));
				firstSeaOverlay.alpha = 0.5;
				firstSeaOverlay.blend = ADD;
		...
		if (curStage == 'beachOne')
			add(firstBeachTrees);
		add(firstBeachLeaves);

		if (curStage == 'beachOneOcean')
			add(bubble);
		// add(rockFlowers);
		add(frontCoral);
		add(firstSeaOverlay);
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 4:
						// Prevents the tweens from playing during the dialogue (must have tween.active = false in the area where the tween was created).

						if (curStage == "beachOne")
						{
							birdsTween.active = true;
						}
						if (curStage == "beachOneOcean")
						{
							fishiesTween.active = true;
							jelliesTween.active = true;
							dolphinsTween.active = true;
							sealsTween.active = true;
						}
					case 5:
				}
	...
	public function playTeleportSound(delay:Float):Void {
		var timeout:Float = 1;
		FlxTween.num(timeout, 0, delay, {onComplete:function(twn:FlxTween)
		{
			FlxG.sound.play(Paths.sound('oceanTeleport'));
		}
		});	
	} 
	...
	var cs_white:FlxSprite;
	var whiteFlash:FlxSprite;
	var cs_bg:FlxSprite;
	var cs_reset:Bool = false;
	var nex:Float = 1;

	public function teleportCutscene()
	{
		inCutscene = true;
		deathCounter = 0;
		cs_cam = new FlxObject(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100, 1, 1);
		cs_zoom = 0.9;
		...
		FlxG.camera.follow(cs_cam, LOCKON, 0.04);
		...
		new FlxTimer().start(0.002, function(tmr:FlxTimer)
		{
			switch (cs_time)
			{
				case 1:
					curStage = 'beachOne';
					cs_cam.x = dad.getMidpoint().x + 150;
					cs_cam.y = dad.getMidpoint().y - 100;
				case 100:
					cs_zoom = 1.0;
					FlxG.sound.play(Paths.sound('oceanTeleport'));
					dad.playAnim('teleport', true);
				case 155:
					cs_zoom = 0.8;
					cs_white = new FlxSprite(-500, -400).makeGraphic(FlxG.width * 4, FlxG.height * 4, FlxColor.WHITE);
					cs_white.scrollFactor.set();
					//cs_white.alpha = 0;
					add(cs_white);

					FlxTween.tween(cs_white, {alpha: 0}, 0.5, {
